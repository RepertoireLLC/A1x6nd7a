const MEDIA_TYPE_KEYWORDS = [
  { type: 'texts', keywords: ['book', 'books', 'text', 'texts', 'manuscript', 'manuscripts', 'document', 'documents', 'paper', 'papers', 'treatise', 'journal', 'journals', 'diary', 'diaries', 'magazine', 'magazines', 'newspaper', 'newspapers', 'transcript', 'transcripts', 'letter', 'letters'] },
  { type: 'image', keywords: ['photo', 'photos', 'photograph', 'photographs', 'image', 'images', 'picture', 'pictures', 'illustration', 'illustrations', 'engraving', 'engravings', 'map', 'maps', 'atlas', 'atlases', 'chart', 'charts'] },
  { type: 'movies', keywords: ['film', 'films', 'movie', 'movies', 'video', 'videos', 'newsreel', 'newsreels', 'footage', 'documentary', 'documentaries', 'tv', 'television'] },
  { type: 'audio', keywords: ['audio', 'recording', 'recordings', 'sound', 'sounds', 'audiobook', 'audiobooks', 'spoken word', 'oral history', 'oral histories', 'interview', 'interviews', 'radio', 'podcast', 'podcasts'] },
  { type: 'software', keywords: ['software', 'program', 'programs', 'application', 'applications', 'app', 'apps', 'game', 'games', 'rom', 'roms'] },
  { type: 'data', keywords: ['dataset', 'datasets', 'data', 'statistics', 'statistical', 'census', 'tables', 'table'] },
  { type: 'web', keywords: ['website', 'websites', 'web page', 'web pages', 'webpage', 'webpages', 'blog', 'blogs', 'site', 'sites', 'snapshot', 'snapshots'] }
];
...
function stripPattern(text, pattern, onMatch) {
  let working = text;
  let match;
  ...
  for (const entry of MEDIA_TYPE_KEYWORDS) {
    for (const keyword of entry.keywords) {
      if (lower.includes(keyword)) {
        const current = scores.get(entry.type) ?? 0;
        scores.set(entry.type, current + 1);
      }
    }
  }
...
function detectSourceTrust(text) {
  const lower = text.toLowerCase();
...
export function interpretSearchQuery(input) {
  ...
  working = stripPattern(
    working,
    /(?:before|earlier than|prior to)\s+(?:the\s+year\s+)?(\d{3,4})/i,
    (match) => {
  ...
  working = stripPattern(
    working,
    /(?:after|later than)\s+(?:the\s+year\s+)?(\d{3,4})/i,
    (match) => {
      const value = Number.parseInt(match[1], 10);
      if (Number.isFinite(value)) {
  ...
  working = stripPattern(
    ...
    (match) => {
      ...
      if (Number.isFinite(base)) {
        ...
        const spanEnd = isCentury ? base + 99 : base + 9;
        applyYearFrom(base);
        applyYearTo(spanEnd);
      }
    }
  ...
  working = stripPattern(
    working,
    /(?:circa|c\.?|around|approximately)\s+(\d{3,4})/i,
    (match) => {
      const value = Number.parseInt(match[1], 10);
      if (Number.isFinite(value)) {
