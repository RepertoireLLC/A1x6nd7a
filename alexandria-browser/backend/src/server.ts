...
const OFFLINE_FALLBACK_ENABLED = getEnv("ARCHIVE_ENABLE_OFFLINE_DATA") === "true";
...
function respondOfflineDisabled(res: ServerResponse, service: string): void {
  sendJson(res, 502, {
    error: `${service} is temporarily unavailable`,
    details:
      "Live Internet Archive services could not be reached and offline fallback data is disabled. Enable ARCHIVE_ENABLE_OFFLINE_DATA=true to use the bundled samples while developing without network access."
  });
}
...
async function handleSearch({ res, url }: HandlerContext): Promise<void> {
  ...
  const getFallbackSearch = () =>
    performLocalArchiveSearch(query, safePage, safeRows, {
  ...
  try {
    ...
    if (!OFFLINE_FALLBACK_ENABLED) {
      respondOfflineDisabled(res, "Internet Archive search");
      return;
    }

    data = getFallbackSearch();
    usedFallback = true;
  }
...
async function handleMetadata({ res, url }: HandlerContext): Promise<void> {
  ...
  try {
    ...
    if (!OFFLINE_FALLBACK_ENABLED) {
      respondOfflineDisabled(res, "Internet Archive metadata");
      return;
    }
  }
...
async function handleCdx({ res, url }: HandlerContext): Promise<void> {
  ...
  try {
    ...
    if (!OFFLINE_FALLBACK_ENABLED) {
      respondOfflineDisabled(res, "Wayback Machine CDX history");
      return;
    }
  }
...
async function handleScrape({ res, url }: HandlerContext): Promise<void> {
  ...
  try {
    ...
    if (!OFFLINE_FALLBACK_ENABLED) {
      respondOfflineDisabled(res, "Archive highlight scrape");
      return;
    }
  }
