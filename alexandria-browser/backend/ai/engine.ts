...
async function loadGenerator(): Promise<TextGenerationPipeline> {
  ...
  if (!generatorPromise) {
    generatorPromise = (pipeline("text-generation", generatorModel) as Promise<unknown>).then(
      (instance: unknown) => {
...
async function loadEmbedder(): Promise<FeatureExtractionPipeline> {
  ...
  if (!embedderPromise) {
    embedderPromise = (pipeline("feature-extraction", embeddingModel) as Promise<unknown>).then(
      (instance: unknown) => {
