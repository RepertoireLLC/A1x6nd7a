...
type FieldName = keyof FieldTexts;
...
type FieldConfigMap = Record<FieldName, FieldConfig>;

const BASE_FIELD_CONFIG: FieldConfigMap = {
...
type FieldOverride = Partial<FieldConfig>;

const MEDIA_TYPE_FIELD_OVERRIDES: Record<string, Partial<Record<FieldName, FieldOverride>>> = {
  texts: {
...
function createBaseFieldConfig(): FieldConfigMap {
  return {
...
function resolveFieldConfig(doc: ArchiveSearchDoc): FieldConfigMap {
  const config = createBaseFieldConfig();
  const mediaType = extractMediaType(doc as unknown as Record<string, unknown>);
  if (!mediaType) {
    return config;
  }
  ...
  for (const [field, override] of Object.entries(overrides) as Array<[FieldName, FieldOverride]>) {
    if (!override) {
      continue;
    }
...
function computeFuzzyMatchScore(words: string[], keyword: string, config: FieldConfig): number {
...
export function scoreArchiveDocTruth(doc: ArchiveSearchDoc, context: TruthScoringContext): TruthScoreResult {
  ...
  const fieldConfig = resolveFieldConfig(doc);
  const relevance = context.originalQuery
    ? computeRelevance(fieldTexts, context.normalizedQuery, context.keywords, fieldConfig)
    : 0.2;
